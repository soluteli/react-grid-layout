...
export function compact(
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    if (!l.static && !noCollisionAndSort) {
...
export function moveElement(
  ...
  // dev: 自由模式，随意拖拽， 无须重新排序
  if (noCollisionAndSort) {
    l.moved = false;
    return layout;
  }
...
export function synchronizeLayoutWithChildren(
  ...
  layout = compact(layout, compactType, cols, noCollisionAndSort);
